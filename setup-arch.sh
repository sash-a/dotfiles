#!/usr/bin/env bash
#
# CachyOS setup script — installs everything described in README.md.
#
# Idempotent: safe to re-run. Each step checks whether it's already done.
# Usage:  bash ~/cachy-setup.sh
#
set -uo pipefail
...
# ----------------------------------------------------------------------------
base_system() {
  log "Updating system and installing base-devel / common utils"
  # Synchronize repositories and upgrade system
  sudo pacman -Syu --noconfirm
  
  # Install essentials (base-devel covers build-essential equivalents)
  sudo pacman -S --needed --noconfirm \
    base-devel procps-ng curl file git wget unzip ca-certificates \
    pkgconf openssl python python-pip htop tree jq fontconfig
...
# ----------------------------------------------------------------------------
# 5. ghostty (terminal) via AUR helper (paru is native to CachyOS)
# ----------------------------------------------------------------------------
setup_ghostty() {
  if have ghostty; then ok "ghostty already installed"; return; fi
  log "Installing ghostty via AUR"
  paru -S --needed --noconfirm ghostty
  ok "ghostty installed"
}

# ----------------------------------------------------------------------------
# 6. VS Code (Official Bin from AUR)
# ----------------------------------------------------------------------------
setup_vscode() {
...
setup_keyd() {
  if ! have keyd; then
    log "Installing keyd from Official Repositories"
...
# Cleans cached uninstalled packages to save space
